class Grid
  ...
  Wire = Struct.new :points
  def add_wire(paths)
    x = y = 0
    wire = Wire.new []
    paths.each do |path|
      case path
      when /R(\d+)/ then $1.to_i.times { y += 1; wire.points << [x, y] }
      when /L(\d+)/ then $1.to_i.times { y -= 1; wire.points << [x, y] }
      when /U(\d+)/ then $1.to_i.times { x += 1; wire.points << [x, y] }
      when /D(\d+)/ then $1.to_i.times { x -= 1; wire.points << [x, y] }
      end
    end
    @wires << wire
  end
  ...
  def connections
    (wires[0].points & wires[1].points).uniq
  end

  def manhatten
    connections.map{|a,b| a.abs + b.abs }.sort
  end

  def wire_lengths_to(pos)
    wires.map do |wire|
      wire.points.index(pos) + 1
    end
  end
end
