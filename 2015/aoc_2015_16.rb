...
AuntSue = Struct.new :name, :children, :cats, :samoyeds, :pomeranians, :akitas, :vizslas, :goldfish, :trees, :cars, :perfumes do
  def self.parse(string)
    properties = string.split(': ').map{ _1.split(', ') }.flatten
    sue = new properties.shift
    until properties.empty?
      sue.send("#{properties.shift}=", properties.shift.to_i)
    end

    sue
  end

  def match1?(compare_list)
    each_pair do |key, value|
      next if key == :name
      next if value.nil?

      return false unless compare_list[key] == value
    end

    true
  end

  def match2?(compare_list)
    each_pair do |key, value|
      next if value.nil?
...
if __FILE__ == $0
  list = DATA.readlines.map { AuntSue.parse(_1.chomp) }

  print "Part 1: "
  puts list.find { _1.match1?(SUE_PATTERN) }.name
  print "Part 2: "
  puts list.find { _1.match2?(SUE_PATTERN) }.name
end
