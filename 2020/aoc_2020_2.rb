def part_1_valid?(actual_pw, min, max, char)
  actual_pw.count(char).between?(min, max)
end

def part_2_valid?(actual_pw, one, two, char)
  (actual_pw[one-1] == char) ^ (actual_pw[two-1] == char)
end

counter = Hash.new(0)

DATA.each do |pw_long|
  pw_long.chomp!
  pw_long =~ /(\d+)-(\d+) (.): (.*)/
  min = $1.to_i
  max = $2.to_i
  ...
  counter[:part_1] += 1 if part_1_valid?(actual_pw, min, max, char)
  counter[:part_2] += 1 if part_2_valid?(actual_pw, min, max, char)
end
