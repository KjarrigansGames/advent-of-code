...
KEYS = %w[byr iyr eyr hgt hcl ecl pid cid].sort.freeze
KEYS_1 = %w[byr iyr eyr hgt hcl ecl pid].sort.freeze
EYE_COLORS = %w[amb blu brn gry grn hzl oth]
def valid?(pass)
  pass.keys.sort & KEYS_1 == KEYS_1
end

def valid2?(pass)
  pass['hgt'] =~ /(\d+)(cm|in)/
  ref = $2 == 'cm' ? [150, 193] : [59, 76]
  $1.to_i.between?(*ref) &&
  pass['byr'].to_i.between?(1920, 2002) &&
  pass['iyr'].to_i.between?(2010, 2020) &&
  pass['eyr'].to_i.between?(2020, 2030) &&
  pass['hcl'].match?(/^#[0-9a-f]{6}$/) &&
  EYE_COLORS.include?(pass['ecl']) &&
  pass['pid'].match?(/^[0-9]{9}$/)
rescue
  false
end
...
DATA.each do |passport|
  passport.chomp!
  if passport.empty?
    list << passport_data.to_h{ _1.split(':') }
    passport_data = []
    next
  end
  passport_data += passport.split(/[,\s]/) # .to_h{ _1.split(_1) })
end
list << passport_data.to_h{ _1.split(':') }

p list.map{_1['pid'] }

# p list.select{ valid?(_1) && valid2?(_1) }.size
# p list
