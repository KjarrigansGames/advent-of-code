@rules = {}
DATA.each do |rule|
  shell, content = rule.split(' bags contain')
  @rules[shell] = content.scan(/((\d+) (.*?) bags?[,\.])/).to_h {  [_1[2], _1[1]] }
end

# Part 1
# valid_bags = ['shiny gold']
# last_bags = valid_bags.size
#
# loop do
#   rules.delete_if do |shell, content|
#     if (content.keys & valid_bags).size > 0
#       valid_bags << shell
#       next true
#     end
#     false
#   end
#   break if last_bags == valid_bags.size
#   last_bags = valid_bags.size
# end
#
# p valid_bags.size - 1
...
def counter(type)
  ...
  contents.map do |typ, value|
    value.to_i + counter(typ) * value.to_i
  end.sum
end

p counter('shiny gold')
